...
export default function IncomingCallsPage() {
  ...
  useEffect(() => {
    setSubtitle(`${incomingCalls.length} incoming call${incomingCalls.length !== 1 ? "s" : ""}`);
  }, [incomingCalls.length, setSubtitle]);
  ...
  const loadCalls = async () => {
    ...
    try {
      ...
      setAllCalls(data || []);
      setIncomingCalls((data || []).filter((call) => call.call_type === "Incoming"));
    } catch (error: any) {
  ...
  const outgoingCount = allCalls.filter((c) => c.call_type === "Outgoing").length;
